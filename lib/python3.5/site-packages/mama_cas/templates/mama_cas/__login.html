{% extends "base.html" %}
{% load i18n %}
 {% block styles %}
    
      <style type="text/css">
       
        .login-title {
          color: #555;
          font-size: 1.4em;
          margin-bottom: 20px;
          margin-top:30px;
        }
        .form-login .checkbox {
          font-weight: normal;
          padding-left: 20px;
        }
        .form-login .form-control {
          font-size: 14px;
          height: auto;
          padding: 10px;
          position: relative;
          -webkit-box-sizing: border-box;
             -moz-box-sizing: border-box;
                  box-sizing: border-box;
        }
        .form-login .form-control:focus {
         # z-index: 2;
        }
        .form-login input[type="text"] {
          margin-bottom: 5px;
          border-bottom-right-radius: 0;
          border-bottom-left-radius: 0;
        }
        .form-login input[type="password"] {
          margin-bottom: 10px;
          border-top-left-radius: 0;
          border-top-right-radius: 0;
        }
         
         .reg{
          display:block;
          margin-left: 25.33333333%;
          width: 50.33333333%;
          margin-top: 20px;
          margin-bottom:30px;
          font-size: 1.1em;
            
         }
      </style>
{% endblock styles %}
{% block content %}
  <h1 class="text-center login-title">
    {% block content_title %}Εισοδος στην Υπηρεσια Ermis-F{% endblock content_title %}
  </h1>

  <div class="row">
    <div class="col-sm-6 col-md-4 col-md-offset-4">
      {% include "mama_cas/_messages.html" %}
      <form class="form-login" method="post">
        {% csrf_token %}
        <label class="sr-only" for="id_username">{{ form.username.label }}</label>
        <input type="text" class="form-control" id="id_username" name="username" placeholder="{{ form.username.label }}" required autofocus>
        <label class="sr-only" for="id_password">{{ form.password.label }}</label>
        <input type="password" class="form-control" id="id_password" name="password" placeholder="{{ form.password.label }}" required>
        {% if form.warn %}
          <label class="checkbox">
            <input type="checkbox" id="id_warn" name="warn"> {{ form.warn.label }}
          </label>
        {% endif %}
        <button class="btn btn-lg btn-primary btn-block" type="submit">Σύνδεση</button><br/>
        <p><a href="http://ermis-floods-dev.aegean.gr/user/password/reset/">Ξέχασα τον κωδικό μου</a></p>
      </form>
    </div>
</div>
   <p class="reg">Αν δεν έχετε λογαριασμό στην Υπηρεσία Ermis-F κάντε κλίκ <a href="http://ermis-floods-dev.aegean.gr/user/register">εδώ</a> για να εγγραφείτε</p>
 
{% endblock content %}
